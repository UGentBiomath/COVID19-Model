...
def draw_function(param_dict, samples_dict):
    ...
    # idx = np.random.randint(low=0, high=len(samples_dict['c_s_NACE21'][0]))
    # param_dict['c_s_NACE21'] = np.array([slice[idx] for slice in samples_dict['c_s_NACE21']], np.float64)
    # param_dict['f_s_NACE21'] = np.array([slice[idx] for slice in samples_dict['f_s_NACE21']], np.float64)
    # # Perturbate l_s
    # param_dict['l_s_1'] =  np.random.normal(loc=1, scale=0.05)*param_dict['l_s_1']
    # param_dict['l_s_2'] =  np.random.normal(loc=1, scale=0.05)*param_dict['l_s_2']
