"""
This script contains a calibration of an PI postponed healthcare model using data from 2020-2021.
"""
...
parser.add_argument("-e", "--end_calibration", help="Calibration enddate. Format 'YYYY-MM-DD'.",default='2020-08-01')
...
parser.add_argument("-ID", "--identifier", help="Name in output files.", default = 'PI_model_test')
...
fig_path = os.path.join(abs_dir,'../../results/QALY_model/postponed_healthcare/calibrations/PI_model/')
# Path where MCMC samples should be saved
samples_path = os.path.join(abs_dir,'../../data/QALY_model/interim/postponed_healthcare/model_parameters/PI_model/')
# Path where samples backend should be stored
backend_folder = os.path.join(abs_dir,'../../results/QALY_model/postponed_healthcare/calibrations/PI_model/')
...
def init_PI_model(start_date,end_date,MDC_sim):
    ...
    params={'covid_H':covid_H,'epsilon':epsilon, 'alpha':alpha, 'Kp':Kp,'Ki':Ki}
    ...
    # Initialize model
    model = PI_Model(init_states,params,coordinates,
...
if __name__ == '__main__':
    ...
    model = init_PI_model(start_date,end_date,MDC_sim)
    ...
    ## Calibrate models ##
    ######################
    labels_dict = {'covid_H':'$covid_H$','epsilon':'$\\epsilon$', 'alpha':'$\\alpha$', 'Kp':'$K_p$','Ki':'$K_I$'}
    ...
    for i,MDC_key in enumerate(MDC_sim):
        ...
        if MDC_key == '04':
            bounds_dict = {'epsilon':(1*10**-10,1),'alpha':(-1,-1*10**-10), 'Kp':(1*10**-10,1),'Ki':(1*10**-10,1)}
            bounds = [bounds_dict[par] for par in pars]
        elif MDC_key == '07':
            bounds_dict = {'epsilon':(1*10**-10,1),'alpha':(-1,1), 'Kp':(1*10**-10,1),'Ki':(1*10**-10,1)}
            bounds = [bounds_dict[par] for par in pars]
        else:
            bounds_dict = {'epsilon':(1*10**-10,1),'alpha':(10**-10,1), 'Kp':(1*10**-10,1),'Ki':(1*10**-10,1)}
            bounds = [bounds_dict[par] for par in pars]

        model = init_PI_model(start_date,end_date,[MDC_key])
        ...
        # --- #
        if n_pso > 0:
            theta = pso.optimize(objective_function, swarmsize=30*len(expanded_bounds), max_iter=n_pso, debug=True, processes=processes,kwargs={'simulation_kwargs':{'method':'LSODA'}})[0]
            ...
            theta = np.array([])
            for param in pars:
                print(model.parameters[param][0])
