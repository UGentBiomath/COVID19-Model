...
+ `average_QALY_losses_per_age_group.csv` and `average_QALY_losses_per_age.csv` contains intermediate data on the average QALY losses per age and hospitalisation group, generated by script `notebooks/preprocessing/woldmuyn_long_covid_average_QALY_loss.py`
...
+ `hospital_yearly_QALYs.csv` copy from `hospital_yearly_QALYs.xlsx` with only essential data and linkage between disease groups and Major Diagnostic Groups.
...
+ `MZG_baseline.csv` contains the average absolute number of patients present in the Ghent University Hospital per Major Diagnostic Group for every week and day in the year.
+ `MZG_residence_times.csv` contains the average length of stay of patients present in the Ghent University Hospital per Major Diagnostic Group.
+ `MDC_dict.csv` contains the MDC numbers and associated Major Diagnostic Group.
